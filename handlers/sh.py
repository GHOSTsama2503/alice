...
@Client.on_message(filters.user(env.OWNER_ID) & filters.command("sh", prefixes=env.PREFIXES))
async def sh_shell_command(client: Client, message: Message):
    ...
    elif len(message.text) == 3 and message.reply_to_message and message.reply_to_message.text:
    ...
    if len(reply_text) >= output_length_limit:
        ...
        caption = f"**Input:**\n`{command}`"
        await message.reply_document(output_file, caption=caption, quote=True, parse_mode=ParseMode.MARKDOWN)
